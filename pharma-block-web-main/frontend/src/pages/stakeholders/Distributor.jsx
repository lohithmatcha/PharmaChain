...
const Distributor = () => {
  ...
  const loadBlockchaindata = async () => {
    ...
    if (networkData) {
      ...
      let curId = 0;
      try{
        curId = await supplychain.methods.findDIS(account).call();
      }
      catch{
      ...
      for (let i = 0; i < retCtr; i++) {
        RETList[i] = await supplychain.methods.RET(i + 1).call();
      }
      setRETlist(RETList);
      ...
      for (let i = 0; i < medCtr; i++) {
        ...
        if (parseInt(med.DISid) === parseInt(curId)) {
          if (medStage === "Manufacturing Stage") tablar.push(med);
          else {
            const newDISData = med.newDIS.split(",");
            const formattedData = {
            ...
            tablar2.push({ id: med.id, name: med.name, description:med.description, newDIS: formattedData });
          }
        }
      }
      setTablar(tablar);
  ...
  return (
    ...
    {showPopup && (
      ...
      {isLoading && <Loader />}
      {errorPage && <ErrorPage />}
      {!errorPage && !isLoading && <div className="stakeholders-page">
        <div className="stakeholders-container">
          <div className="stakeholder-dashboard">
            <div className="dashboard-header">
              <h2 className="welcome-text">Welcome, Distributor</h2>
            </div>
                        ...
                        <tr key={index}>
                          ...
                          <td>
                            <form onSubmit={(e) => handleSubmit(e, medicine.id)}>
                              <button disabled={DisBtn[medicine.id] < 1} type="submit" className="submit-button">
            ...
            {showPopup && (
              <div className="popup2">
                Medicine Distributed and Status Updated ✅
              </div>
            )}
                    ...
                    <tbody>
                      {Tablar2.map((medicine, index) => (
                        <tr key={index}>
                          ...
                          <td>{medicine.description}</td>
                          <td>{medicine.newDIS.latitude}</td>
                          <td>{medicine.newDIS.longitude}</td>
                          <td>{medicine.newDIS.date}</td>
                          <td>{medicine.newDIS.time}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
